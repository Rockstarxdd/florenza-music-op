...
@Client.on_callback_query(
    filters.regex(pattern=r"^(ytplay|pause|skip|leave|puse|resume|menu|cls)$")
)
@cb_admin_check
async def m_cb(b, cb):
    global que   
    ...
    if type_ == "pause":
        ...
        now_playing = temp[0][0]
        by = temp[0][1].mention(style="md")
        msg = "**Florenza Started** Mm {}".format(cb.message.chat.title)
        msg += "\n• "+ now_playing
        msg += "\n• Atas permintaan "+by
        temp.pop(0)
        if temp:
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    # print(e)
                    await lel.edit(
                        f"<b>⛑ Flood Wait Error ⛑\n{user.first_name} Florenza helper is not in Ur chat plz add Manually."
                        f"\n\n Assistant @{ASSISTANT_NAME} try to add manually</b>",
                    )
    try:
    ...
    if audio:
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    ...
                    InlineKeyboardButton("📣 ᴄʜᴀɴɴᴇʟ", url=f"https://t.me/Florenza_Updates")
                ],
            ]
        )
        ...
        try:
            ...
            while j < 6:
                ...
                toxxt += f" └ ⚡ __ᴘᴏᴡᴇʀᴇᴅ ʙʏ {BOT_NAME}__\n\n"
                j += 1            
            keyboard = InlineKeyboardMarkup(
                [
                    [
...
async def ytplay(_, message: Message):
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    # print(e)
                    await lel.edit(
                        f"<b>ꜰʟᴏᴏᴅ ᴡᴀɪᴛ ᴇʀʀᴏʀ\n{user.first_name}  ʜᴇʟᴘᴇʀ ɪꜱ ɴᴏᴛ ɪɴ ᴜʀ ᴄʜᴀᴛꜱ."
    ...
    for i in message.command[1:]:
        query += " " + str(i)
    print(query)
    await lel.edit("🎵 **𝙲𝚘𝚗𝚗𝚎𝚌𝚝𝚒𝚗𝚐 𝚝𝚘 𝚝𝚑𝚎 𝚏𝚕𝚘𝚛𝚎𝚗𝚣𝚊 𝚜𝚎𝚛𝚟𝚎𝚛..**")
    ydl_opts = {"format": "bestaudio[ext=m4a]"}
    try:
        results = YoutubeSearch(query, max_results=1).to_dict()
        url = f"https://youtube.com{results[0]['url_suffix']}"
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                ...
                InlineKeyboardButton("📣 ᴄʜᴀɴɴᴇʟ", url=f"https://t.me/Florenza_Updates"),
                InlineKeyboardButton("✨ ɢʀᴏᴜᴘ", url=f"https://t.me/Florenza_Updates")
            ],
        ]
    )
    requested_by = message.from_user.first_name
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        await message.reply_photo(
            ...
            caption = f"🏷 **ɴᴀᴍᴇ:** [{title[:25]}]({url})\n⏱ **ᴅᴜʀᴀᴛɪᴏɴ:** `{duration}`\n😍 **ꜱᴛᴀᴛᴜꜱ:** `Qᴜᴇᴜᴇᴅ ɪɴ ᴘᴏꜱɪᴛɪᴏɴ {position}`\n" \
                    + f"🎧 **ᴜꜱᴇʀ ʙʏ:** {message.from_user.mention} join @Florenza_support",
                   reply_markup=keyboard,
        )
        os.remove("final.png")
        return await lel.delete()
        ...
        await message.reply_photo(
            photo="final.png",
            caption = f"🏷 **ɴᴀᴍᴇ:** [{title[:25]}]({url})\n⏱ **ᴅᴜʀᴀᴛɪᴏɴ:** `{duration}`\n💡 **ꜱᴛᴀᴛᴜꜱ:** `ᴘʟᴀʏɪɴɢ`\n" \
                    + f"🎧 **Request by:** {message.from_user.mention} join @Florenza_support",
                   reply_markup=keyboard,)
        os.remove("final.png")
        return await lel.delete()
